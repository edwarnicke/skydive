...
type JSONGroup struct {
	GroupID uint      `json:"group_id"`       // id of the group
...
type Action struct {
	Action    string    `json:"f"`           // Action name
	Arguments []*Action `json:"a,omitempty"` // Arguments if it exists
	Key       string    `json:"k,omitempty"` // Key for aguments such as k=v
}

// Filter is an elementary filter in an openflow rule
type Filter struct {
	Key   string `json:"k"`           // left hand side
	Value string `json:"v"`           // right hand side
	Mask  string `json:"m,omitempty"` // mask if used
}

// Meta is anything not a filter or an action always as a pair key/value
type Meta struct {
	Key   string `json:"k"` // key
	Value string `json:"v"` // raw value
}
